...
/* ------------------------------------------------------------------ *
 * is this token a mnemonic (an opcode / directive / trap alias)?
 *
 * We need this to answer the first parsing question on every line:
 * "is the first word a LABEL, or the MNEMONIC itself?" A line may start
 * with an optional label, so the only way to tell `LOOP` from `ADD` is
 * to know the set of real mnemonics.
 *
 ...
 * ------------------------------------------------------------------ */
static Operand classify_operand(const char *token) {
  Operand operand;
...
InstructionIntermediary *parse_line(const char *line) {
  ...
  InstructionIntermediary *intermediary = calloc(1, sizeof *intermediary);
  intermediary->raw = raw;
  ...
  /* (c) peel the first two whitespace-separated words. strtok_r hands back a
         ...
         is exactly the operand region once we've taken the mnemonic. */
  char *save = NULL;
  ...
  if (is_mnemonic(first)) {
    ...
    intermediary->label = strdup(first);
    char *second = strtok_r(NULL, " \t", &save);
    mnemonic = second;              /* may be NULL: a label sitting on its own line */
    operand_region = save;
  }

  if (mnemonic != NULL) {
    intermediary->mnemonic = strdup(mnemonic);
  }

  /* (d) parse the operands. */
  char *upper_mnemonic = mnemonic ? to_upper_copy(mnemonic) : NULL;

  if (upper_mnemonic && strcmp(upper_mnemonic, ".STRINGZ") == 0) {
    ...
    if (open && close && close > open) {
      ...
      text[len] = '\0';
      intermediary->operands[0].type = OPERAND_STRING;
      intermediary->operands[0].as.string = text;
      intermediary->operands_length = 1;
    }
    ...
    if (op_trimmed[0] != '\0') {
      ...
      for (uint16_t i = 0; i < pieces->length && intermediary->operands_length < MAX_OPERANDS; i++) {
        intermediary->operands[intermediary->operands_length] = classify_operand(pieces->data[i]);
        intermediary->operands_length++;
      }
...
void free_instruction_intermediary(InstructionIntermediary *intermediary) {
  ...
  for (int i = 0; i < intermediary->operands_length; i++) {
    switch (intermediary->operands[i].type) {
      case OPERAND_LABEL:  free(intermediary->operands[i].as.label);  break;
      case OPERAND_STRING: free(intermediary->operands[i].as.string); break;
      case OPERAND_REGISTER:
      case OPERAND_INTEGER:
        break;  /* nothing owned */
    }
  }
  free(intermediary->raw);
  free(intermediary->label);
  free(intermediary->mnemonic);
  free(intermediary->machine_codes);
  free(intermediary);
}

void print_instruction_intermediary(const InstructionIntermediary *intermediary) {
  printf("label=%-10s mnemonic=%-10s operands=[",
         intermediary->label ? intermediary->label : "-",
         intermediary->mnemonic ? intermediary->mnemonic : "-");

  for (int i = 0; i < intermediary->operands_length; i++) {
    const Operand *op = &intermediary->operands[i];
    if (i > 0) {
      printf(", ");
    }
    switch (op->type) {
...
static void symbol_table_add(SymbolTable *table, const char *label, uint16_t address) {
...
static int symbol_table_lookup(const SymbolTable *table, const char *label, uint16_t *out) {
...
/* --- how many program words does this line occupy? (drives the location
   counter in pass 1 and the emit size in pass 2). --- */
static int line_word_count(const InstructionIntermediary *line, const char *upper) {
...
static uint16_t pc_offset(const SymbolTable *table, const char *label,
                          uint16_t instr_addr, int bits, int *ok) {
  uint16_t target;
  if (!symbol_table_lookup(table, label, &target)) {
...
static uint16_t reg_code(const Operand *op) { return op->as.reg->code; }
...
static uint16_t encode_instruction(const InstructionIntermediary *line,
  ...
  if (strcmp(upper, "ADD") == 0 || strcmp(upper, "AND") == 0) {
    ...
    uint16_t word = (uint16_t) ((opc << 12) | (reg_code(&op[0]) << 9) | (reg_code(&op[1]) << 6));
    if (op[2].type == OPERAND_REGISTER) {
      word |= reg_code(&op[2]);                     /* bit 5 = 0, SR2 in bits 2-0 */
    } else {
      word |= (uint16_t) ((1 << 5) | (op[2].as.integer & 0x1F));  /* bit 5 = 1, imm5 */
    }
    return word;
  }

  if (strcmp(upper, "NOT") == 0)
    return (uint16_t) ((0x9 << 12) | (reg_code(&op[0]) << 9) | (reg_code(&op[1]) << 6) | 0x3F);
  ...
  if (upper[0] == 'B' && upper[1] == 'R') {
    ...
    return (uint16_t) ((0x0 << 12) | (nzp << 9) | pc_offset(table, op[0].as.label, line->address, 9, ok));
  }

  if (strcmp(upper, "JMP") == 0) return (uint16_t) ((0xC << 12) | (reg_code(&op[0]) << 6));
  if (strcmp(upper, "RET") == 0) return (uint16_t) ((0xC << 12) | (0x7 << 6));  /* JMP R7 */

  if (strcmp(upper, "JSR")  == 0)
    return (uint16_t) ((0x4 << 12) | (1 << 11) | pc_offset(table, op[0].as.label, line->address, 11, ok));
  if (strcmp(upper, "JSRR") == 0)
    return (uint16_t) ((0x4 << 12) | (reg_code(&op[0]) << 6));
  ...
  for (size_t i = 0; i < sizeof pcrel / sizeof pcrel[0]; i++) {
    if (strcmp(upper, pcrel[i].name) == 0)
      return (uint16_t) ((pcrel[i].opc << 12) | (reg_code(&op[0]) << 9)
                         | pc_offset(table, op[1].as.label, line->address, 9, ok));
  }

  /* base+offset load/stores: opcode | DR/SR | BaseR | offset6. */
  if (strcmp(upper, "LDR") == 0 || strcmp(upper, "STR") == 0) {
    uint16_t opc = (upper[0] == 'L') ? 0x6 : 0x7;
    return (uint16_t) ((opc << 12) | (reg_code(&op[0]) << 9) | (reg_code(&op[1]) << 6)
                       | (op[2].as.integer & 0x3F));
  }

  if (strcmp(upper, "TRAP") == 0) return (uint16_t) ((0xF << 12) | (op[0].as.integer & 0xFF));
...
static int emit_line(InstructionIntermediary *line, const char *upper, const SymbolTable *table) {
  ...
  if (strcmp(upper, ".FILL") == 0) {
    ...
    if (line->operands[0].type == OPERAND_LABEL) {
      if (!symbol_table_lookup(table, line->operands[0].as.label, &value)) {
  ...
  int ok = 1;
  uint16_t word = encode_instruction(line, upper, table, &ok);
  line->machine_codes = malloc(sizeof(uint16_t));
...
static int assemble(InstructionIntermediary **program, int count, uint16_t *origin_out) {
  SymbolTable table = {0};
  ...
  for (int i = 0; i < count; i++) {
    InstructionIntermediary *line = program[i];
    char *upper = line->mnemonic ? to_upper_copy(line->mnemonic) : NULL;
    ...
    line->address = lc;
    if (line->label) {
      symbol_table_add(&table, line->label, lc);
    }
    lc = (uint16_t) (lc + line_word_count(line, upper));
    free(upper);
  }
...
static void put_word(FILE *out, uint16_t word) {
  fputc((word >> 8) & 0xFF, out);
  fputc(word & 0xFF, out);
}

static int write_object_file(const char *path, uint16_t origin,
  ...
  put_word(out, origin);                          /* header: the load address */
  for (int i = 0; i < count; i++) {
    for (int j = 0; j < program[i]->machine_codes_length; j++) {
      put_word(out, program[i]->machine_codes[j]);
    }
  }
  fclose(out);
  return 1;

...
static void test_operation_ld(void) {
  ...
  registers_local[REGISTER_PROGRAM_COUNTER.code] = 0;
  memory_local[4] = 0xABCD;
  /* LD R0, #4: op=0010 dr=000 PCoffset9 */
  uint16_t instruction = (0x2 << 12) | (REGISTER_R0.code << 9) | 0x004;
  operation_ld(instruction, registers_local, memory_local);
  assert(registers_local[REGISTER_R0.code] == 0xABCD);
  assert(registers_local[REGISTER_PROCESSOR_STATUS.code] == CONDITION_FLAG_NEGATIVE); /* high bit set */
}
...
static void test_operation_ldi(void) {
  ...
  uint16_t instruction = (0xA << 12) | (REGISTER_R0.code << 9) | 0x004;
  operation_ldi(instruction, registers_local, memory_local);
  assert(registers_local[REGISTER_R0.code] == 0x0042);
  assert(registers_local[REGISTER_PROCESSOR_STATUS.code] == CONDITION_FLAG_POSITIVE);
}
...
static void test_operation_ldr(void) {
  uint16_t registers_local[REGISTER_COUNT] = {0};
  uint16_t memory_local[16] = {0};
  registers_local[REGISTER_R1.code] = 2;
  memory_local[5] = 0x0007;
  /* LDR R0, R1, #3: op=0110 dr=000 baseR=001 offset6 */
  uint16_t instruction = (0x6 << 12) | (REGISTER_R0.code << 9) | (REGISTER_R1.code << 6) | 0x03;
  operation_ldr(instruction, registers_local, memory_local);
  assert(registers_local[REGISTER_R0.code] == 0x0007);
  assert(registers_local[REGISTER_PROCESSOR_STATUS.code] == CONDITION_FLAG_POSITIVE);
}
...
static void test_operation_st(void) {
  ...
  uint16_t memory_local[16] = {0};
  registers_local[REGISTER_PROGRAM_COUNTER.code] = 0;
  registers_local[REGISTER_R0.code] = 0x1234;
  /* ST R0, #4: op=0011 sr=000 PCoffset9 */
  uint16_t instruction = (0x3 << 12) | (REGISTER_R0.code << 9) | 0x004;
...
static void test_operation_sti(void) {
  ...
  uint16_t memory_local[16] = {0};
  registers_local[REGISTER_PROGRAM_COUNTER.code] = 0;
  registers_local[REGISTER_R0.code] = 0x9999;
  memory_local[4] = 8;  /* pointer */
  /* STI R0, #4: op=1011 sr=000 PCoffset9 */
  uint16_t instruction = (0xB << 12) | (REGISTER_R0.code << 9) | 0x004;
...
static void test_operation_str(void) {
  ...
  uint16_t memory_local[16] = {0};
  registers_local[REGISTER_R1.code] = 2;
  registers_local[REGISTER_R0.code] = 0x5678;
  /* STR R0, R1, #3: op=0111 sr=000 baseR=001 offset6 */
  uint16_t instruction = (0x7 << 12) | (REGISTER_R0.code << 9) | (REGISTER_R1.code << 6) | 0x03;
